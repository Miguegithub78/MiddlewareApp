...
const Home = () => {
  ...
  let [page, setPage] = useState(0);
   ...
   const pagination = () => {
    if (juniors.length) return juniors.slice(page, page + 4);
    if (juniors.info) return juniors;
    if (companies.length) return companies.slice(page, page + 4);
   ...
   const nextPage = () => {
     if (companies.length > page + 4) {
       setPage(page + 4);
     }
     if (juniors.length > page + 4) {
      setPage(page + 4);
    }
   };
   const previusPage = () => {
